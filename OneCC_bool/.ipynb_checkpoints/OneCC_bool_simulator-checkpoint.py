...
class OneCC_bool_simulator(object): 
    ...
    def calc_production_rate(self, prev_exp, TG, network_model):
        TG_gene_obj = network_model.network_dict[TG]
        reg_coefficient = TG_gene_obj.regulation_coeff
        
        def calc_scale_expression(raw_exp, TF_norm_factors):
        ...
        norm_dict = dict()
        for TF in TG_gene_obj.upstream_genes:
            TF_gene_obj = network_model.network_dict[TF]
            TF_norm_factors = network_model.network_dict[TF].norm_factors
            scaled_exp = calc_scale_expression(prev_exp[TF], TF_norm_factors)
        ...
        if len(sigmoid_parameters) == 0:
            linear_parameters = TG_gene_obj.linear_parameters
            total_prob = linear_parameters['m'] * activation_prob * repression_prob + linear_parameters['b']
            ...
            reg_prob = activation_prob * repression_prob
            m = sigmoid_parameters['m']
            b = sigmoid_parameters['b']
            
            total_prob = 1 / (1 + np.exp(-m * (reg_prob + b)))
            print(total_prob)
            return reg_coefficient * total_prob
        ...
        for time_point in time_df.columns:
            if time_point == 0: 
                ...
                for gene_name in network_model.keys():
                    gene_production = self.calc_production_rate(prev_exp, gene_name, self.networks_compilation[initial_subnet])
                    ...
                    if stochasticity == True: 
                        noise_p = np.random.normal(0, t_interval) * (prev_exp[gene_name] ** 0.5)
                    ...
                    time_df.loc[gene_name, time_point] = gene_exp_t
